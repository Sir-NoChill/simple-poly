...
This is an example of an out-of-tree [MLIR](https://mlir.llvm.org/) dialect along with a gazprea `opt`-like tool to operate on that dialect.
...
cmake --build . --target check-gazprea
```
...
cmake -G Ninja `$LLVM_SRC_DIR/llvm` \
    ...
    -DLLVM_EXTERNAL_PROJECTS=gazprea-dialect -DLLVM_EXTERNAL_GAZPREA_DIALECT_SOURCE_DIR=../
cmake --build . --target check-gazprea
```
